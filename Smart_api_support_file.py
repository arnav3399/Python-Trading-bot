...
import pyotp
token= "RH3542WV7XHTALTLSTIF4Z37YE"
...
def angelbrok_login():
    try:
        ...
        obj = SmartConnect(api_key="OzdlFS1e")


        data = obj.generateSession(client_code,password,pyotp.TOTP(token).now())
...
def get_historical_data(name, interval, timeperiod):
	...
	try:
		...
		for data in response['data']:
			...
			historic_df = pd.concat([historic_df, pd.DataFrame([dict1])], ignore_index=True)
